...
void Omin_pudelko_L(char silnik_a, char silnik_b, int obrot1, int prosta1, int obrot2, int prosta2, int obrot3, int prosta3, int obrot4){
	Predkosc(silnik_a,silnik_b);
	Stop();
	Lewo();
	_delay_ms(obrot1);
...
void Omin(char silnik_a, char silnik_b, int obrot1, int prosta1, int obrot2, int prosta2, int obrot3){
	Predkosc(silnik_a,silnik_b);
	Stop();
	Lewo();
	_delay_ms(obrot1);
...
ISR(INT6_vect){
    if((r_mode == 0) && !r_tx){
    	...
    	if(pass){
    		if (licznik_1ms >= 2) ramka |= 0x00000001;
    		else ramka &= ~(0x00000001);
    		if(bit_licznik < 31) ramka <<= 1;
...
void Blink(int czekaj){
	...
	while(running){
		...
		if(pilot == 152) { Cofaj(); pilot = 0; }
		else if(pilot == 24) { Jedz(); pilot = 0; }
		else if(pilot == 216) { Prawo(); pilot = 0; }
		else if(pilot == 88) { Lewo(); pilot = 0; }
		else if(pilot == 176) { if(speed_A<255) speed_A++; pilot = 0;
				LCD_GoTo(0, 1);
				printf("%3d          %3d",speed_A,speed_B);
			}
		else if(pilot == 66) { if(speed_A>0) speed_A--; pilot = 0;
				LCD_GoTo(0, 1);
				printf("%3d          %3d",speed_A,speed_B);
			}
		else if(pilot == 136) { if(speed_B<255) speed_B++; pilot = 0;
				LCD_GoTo(0, 1);
				printf("%3d          %3d",speed_A,speed_B);
			}
		else if(pilot == 98) { if(speed_B>0) speed_B--; pilot = 0;
				LCD_GoTo(0, 1);
					printf("%3d          %3d",speed_A,speed_B);
				}
		else if(pilot != 0) Stop();
...
void Test(void){
	while(running){
		if(!GET(SWITCH1)) CLR(LED1); else SET(LED1);
		if(!GET(SWITCH2)) CLR(LED2); else SET(LED2);
		if(!GET(SWITCH3)) CLR(LED3); else SET(LED3);
		if(!GET(SWITCH4)) CLR(LED4); else SET(LED4);

		if(!GET(BUTTON_U)) CLR(LED5); else SET(LED5);
		if(!GET(BUTTON_D)) CLR(LED6); else SET(LED6);
		if(!GET(BUTTON_L)) CLR(LED7); else SET(LED7);
		if(!GET(BUTTON_R)) CLR(LED8); else SET(LED8);
	}
}
/********************************************************************************/
/********************************************************************************/
...
void Run(char nr){
	...
	case 4:
		...
		while(running){
			if(!GET(INPUT1)) { CLR(LED1); Beep(1500,200); Cofaj(); _delay_ms(400); Prawo(); _delay_ms(500); Jedz(); }
			else SET(LED1);
			if(!GET(INPUT2)) { CLR(LED8); Beep(2000,200); Cofaj(); _delay_ms(400); Lewo(); _delay_ms(500); Jedz(); }
			else SET(LED8);
			if(!GET(BUTTON_L) || !GET(BUTTON_D)) running = 0;
		}
	...
	case 5:
		...
		while(GET(BUTTON_U) && GET(BUTTON_L)){
			if(pilot == 24) {
				pilot = 0;
				if(czulosc != 255) czulosc += 1;
				LCD_GoTo(0, 1);
				printf("czulosc = %3d   ",czulosc);
			}
			if(pilot == 152) {
				pilot = 0;
				if(czulosc != 0) czulosc -= 1;
				LCD_GoTo(0, 1);
				printf("czulosc = %3d   ",czulosc);
			}
		}
	...
	case 6:
		...
		Predkosc(180,180);
		while(running){
			if(!GET(BUTTON_D)) Cofaj();
			else if(!GET(BUTTON_U)) Jedz();
			else if(!GET(BUTTON_R)) Prawo();
			else if(!GET(BUTTON_L)) Lewo();
			else Stop();
		}
		break;
	case 7:
	...
	case 9:
		...
		while(pilot != 104){
			...
			// obrot1
			if(pilot == 128) { obrot1 += 5;
					LCD_GoTo(0, 1);
					printf("obrot1 = %4d   ",obrot1);
					pilot = 0;
				}
			if(pilot == 32) { obrot1 -= 5;
					LCD_GoTo(0, 1);
					printf("obrot1 = %4d   ",obrot1);
					pilot = 0;
				}
			// prosta1
			if(pilot == 64) { prosta1 += 10;
					LCD_GoTo(0, 1);
					printf("prosta1 = %4d  ",prosta1);
					pilot = 0;
				}
			if(pilot == 160) { prosta1 -= 10;
					LCD_GoTo(0, 1);
					printf("prosta1 = %4d  ",prosta1);
					pilot = 0;
				}
			// obort2
			if(pilot == 192) { obrot2 += 5;
					LCD_GoTo(0, 1);
					printf("obrot2 = %4d   ",obrot2);
					pilot = 0;
				}
			if(pilot == 96) { obrot2 -= 5;
					LCD_GoTo(0, 1);
					printf("obrot2 = %4d   ",obrot2);
					pilot = 0;
				}
			// prosta2
			if(pilot == 224) { prosta2 += 10;
					LCD_GoTo(0, 1);
					printf("prosta2 = %4d  ",prosta2);
					pilot = 0;
				}
			if(pilot == 208) { prosta2 -= 10;
					LCD_GoTo(0, 1);
					printf("prosta2 = %4d  ",prosta2);
					pilot = 0;
				}
			// obrot3
			if(pilot == 144) { obrot3 += 5;
					LCD_GoTo(0, 1);
					printf("obrot3 = %4d   ",obrot3);
					pilot = 0;
				}
			if(pilot == 80) { obrot3 -= 5;
					LCD_GoTo(0, 1);
					printf("obrot3 = %4d   ",obrot3);
					pilot = 0;
				}
			// prosta3
			if(pilot == 56) { prosta3 += 10;
					LCD_GoTo(0, 1);
					printf("prosta3 = %4d  ",prosta3);
					pilot = 0;
				}
			if(pilot == 184) { prosta3 -= 10;
					LCD_GoTo(0, 1);
					printf("prosta3 = %4d  ",prosta3);
					pilot = 0;
				}
			// obrot4
			if(pilot == 194) { obrot4 += 5;
					LCD_GoTo(0, 1);
					printf("obrot4 = %4d   ",obrot4);
					pilot = 0;
				}
			if(pilot == 34) { obrot4 -= 5;
					LCD_GoTo(0, 1);
					printf("obrot4 = %4d   ",obrot4);
					pilot = 0;
				}
		...
		while(running){
			...
			LCD_GoTo(0, 0);
			printf("odleglosc = %4d",odleglosc);
			if(odleglosc > 750) Omin_pudelko_L(silnik_a,silnik_b,obrot1,prosta1,obrot2,prosta2,obrot3,prosta3,obrot4);

			if(pilot == 8) { running = 0; pilot = 0; }
		}
		Stop();
		Predkosc(0,0);
		cli();
	...
	case 10:
		...
		while(running){
			...
			LCD_GoTo(0, 0);
			printf("odleglosc = %4d",odleglosc);
			if(odleglosc > 750) Omin_pudelko_P(silnik_a,silnik_b,obrot1,prosta1,obrot2,prosta2,obrot3,prosta3,obrot4);
