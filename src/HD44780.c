...
int LCD_WD(char dataToWrite, FILE * f) {
    LCD_RS_PORT |= LCD_RS;
    _LCD_Write(dataToWrite);
	return(0);
}
...
{	
	if(val == 0)
	{
		LCD_WriteData('0');	// wypisz '0' jeœli podano val=0
	}
	else
	{
		static char buf[5] = {0};	// alokacja u¿ywanego bufora, dla d³.=5 poprawnie wyœwietla bajt :P
		uint8_t i = 3;	// zmienna pomocnicza, musi byæ zainicjowana wartoœci¹ o 2 mniejsz¹ od d³ugoœci bufora
		
		for(uint8_t j=0;j<5;j++)
		{
			buf[j]=0;		
		}
		for(; val && i ; --i, val /= 10)	// odt¹d jest magic
		{
			buf[i] = (val % 10) + '0';
		}
		
		LCD_WriteText(&buf[i+1]);	// wypisz wynik na LCD
	}
}
//-------------------------------------------------------------------------------------------------
//
// Wypisanie liczby uint8_t w formacie 3 cyfrowym
//
//-------------------------------------------------------------------------------------------------
void LCD_write_byte0(uint8_t val)
{
	if(val < 10) LCD_WriteText("00");	// jeœli liczba <10, wypisz "00"
	else if (val<100)_LCD_Write('0');   // jeœli liczba <100, wypisz '0'
	LCD_write_byte(val);	// wypisz resztê
}
//-------------------------------------------------------------------------------------------------
//
